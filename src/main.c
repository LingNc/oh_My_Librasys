...
void borrow_book(size_t student_id) {
    clear_screen();
    printf("借书功能\n");
    size_t book_id;
    printf("请输入书籍ID: ");
...
void view_borrow_info(size_t student_id) {
    clear_screen();
    printf("查看借阅信息功能\n");
...
void student_menu(size_t student_id) {
    int highlight = 0;
    int choice = -1;
    char c;

    while (1) {
        display_student_menu(highlight);
        char line[100] = { 0 };
...
void book_management_menu() {
    ...
    while (1) {
        display_book_management_menu(highlight);
        char line[100] = { 0 };
        fgets(line, 100, stdin);
        if (sscanf(line, "%d", &choice) == 1) {
            switch (choice) {
...
int main() {
    ...
    bookDb = database("Book.txt", Book);
    studentDb = database("Student.txt", Student);
    borrowDb = database("borrow_records", String);
