...
void stu_list(Student* student_set)
{
    ...
    mvwprintw(main_win, 0, 40, "%s", "所属");
    mvwprintw(main_win, 0, 55, "%s", "借阅数量");
    mvwprintw(main_win, 0, 90, "%s", "逾期数量");
    attroff(COLOR_PAIR(PAD_HEIGHT_LIGHT));
...
void stu_borrow_list(Student* stu)
{
    ...
    int info_start_x = 1;
    WINDOW *info_win = creat_win(info_height, info_width, info_start_y, info_start_x);
    mvwprintw(info_win, 0, (terminal.width - 2 - strlen("图书借阅界面")) / 2, "图书借阅界面");
    //当前书籍信息打印
    MenuInfo info = {150, 100, "admin", "2024-12-17"};
    print_info(info_win,&infoname, &info);
    wrefresh(info_win);
...
// 创建 pad
    int pad_height = LIST_SIZE + 1;
    int pad_width = terminal.width - 2; // 宽度减少2
    WINDOW *pad = newpad(pad_height + 5, pad_width);
    keypad(pad, TRUE);
REFRESH_PAD:
    // 绘制列表项到 pad 上
    for (int i = 0; i < LIST_SIZE; i++) {
    ...
    // 刷新 pad 的初始视图
    prefresh(pad, scroll_offset + 5, 0, main_win_start_y + 1, main_win_start_x + 1, main_win_start_y + main_win_height - 2, main_win_start_x + main_win_width - 2);
    wrefresh(main_win);
